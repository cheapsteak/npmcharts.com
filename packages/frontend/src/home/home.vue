  ...
  >
    ...
    >
      <h1 class="heading">
        ...
        <a
          v-else
          :href="
            $route.params.packages ? '/compare/' + $route.params.packages : '/'
          "
          class="identity"
          title="npmcharts"
        >
        ...
        <p class="sub-heading">
          <template v-if="!isLoadingNpmMetaData && packageNames.length > 1"
            >compare</template
          >
          <template v-if="isLoadingNpmMetaData">
            fetching package release dates..
          </template>
          <span v-else class="package-entry" v-for="moduleName in packageNames">
            <a
              class="package-name"
            ...
            <svg
              v-if="npmMetadataByPackageName[moduleName].hasTypings"
      ...
      <form class="header-controls-wrapper">
        ...
        <div class="graph-config">
          <router-link
            ...
            v-if="packageNames"
            :to="{
              path: '/compare/' + packageNames.join(','),
              query: getMergedQueryParams({ interval: 1 }),
            }"
          ...
          <router-link
            ...
            :to="{
              path: '/compare/' + packageNames.join(',') + '?',
              query: getMergedQueryParams({ interval: 7 }),
            }"
            @click.native="track('set interval', interval)"
          >
          ...
          <router-link
            ...
            :to="{
              path: '/compare/' + packageNames.join(',') + '?',
              query: getMergedQueryParams({ interval: 30 }),
            }"
            @click.native="track('set interval', interval)"
          >
    ...
    >
      <div class="chart-container">
                ...
                :style="{
                  color:
                    packageNames && packageNames.indexOf(packageItem) > -1
                      ? palette[
                          packageNames.indexOf(packageItem) % palette.length
                        ]
                      : '',
                }"
                >{{ packageItem }}</span
        ...
        <div
          class="no-packages-selected"
          v-if="!packageDownloadStats && !isLoadingDownloadStats"
        >
    ...
    <footer>
      <div class="about">
        <span class="created-by" @click="handleClickContributor">
          Crafted in {{ contributorInfo.location }} by
...
export default {
  created() {
    ...
    this.getPackagesDownloadDataByNames(
      this.packageNames,
  ...
  watch: {
    ...
    packages(value) {
      this.handlePackagesChange();
    },
  },
  provide() {
    const removePackage = this.removePackage;
    const addPackage = this.addPackage;
    return {
      removePackage,
      addPackage,
    };
  ...
  methods: {
    ...
    handlePackagesChange() {
      const nextRouteSansQuery = `/compare/${this.packages.join(',')}`;
    ...
    addPackage(name, notify = true) {
      this.packages.indexOf(name) === -1 && this.packages.push(name);
    },
    removePackage(name, notify = true) {
      this.packages.splice(this.packages.indexOf(name), 1);
    },
    setPackages(val, notify = true) {
      this.packages.splice(0, this.packages.length, ...val);
    },
    async getPackagesDownloadDataByNames(names, start, end) {
        setTimeout(() => ga('send', 'pageview'));

        // set notify to false to prevent triggering route change
        this.setPackages(names, false);

        const operation = _.every(names, isPackageName)
            ? // names are npm packages
            this.getPackagesDownloadsOverPeriod(names, start, end)
            : // names are github repo names
            this.fetchReposCommitsStats(names);
